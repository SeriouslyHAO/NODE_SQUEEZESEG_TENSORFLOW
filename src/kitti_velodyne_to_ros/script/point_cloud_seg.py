...
class NPY_TENSORFLOW_TO_ROS():
    def __init__(self, pub_topic, FLAGS, npy_path="", npy_file_list=""):
        ...
        rospy.Subscriber('/ss_filtered', PointCloud2, self.callback_pointcloud)
    ...
    def callback_pointcloud(self, data):
        ...
        # )
        #
        # lidar=np.tile(lidar,(64,1,1))
        # lidar_mask=np.tile(lidar_mask,(64,1,1))
        ...
        print 'lidar_mask.shape ' + str(lidar_mask.shape)

        # lidar = (lidar - self._mc.INPUT_MEAN) / self._mc.INPUT_STD
        #

        pred_cls = self._session.run(
            self._model.pred_cls,
            feed_dict={
                self._model.lidar_input: [lidar],
